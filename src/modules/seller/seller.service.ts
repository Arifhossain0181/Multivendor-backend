...
export const createSellerProfile= async(
    ...
    try{
        ...
        if (existingSeller) {
            throw new ApiError(400,"seller", "Seller profile already exists for this user");
        }
        ...
        throw new ApiError(500, "occured","An error occurred while creating the seller profile");
    }
}
...
export const findById =async (userId: string) => {
    try {
        ...
        throw new  ApiError(500,"an ", "An error occurred while fetching the seller profile");
    }
}
...
export const transitionSubOrder = async (subOrderId :string,sellerId:string,nextStatus:string) => {
   ...
   try{
    ...
    if(subOrder.sellerId !== sellerId){
        throw new ApiError(403,"new", "You are not authorized to update this sub-order");
    }

    // master order Paid nh hole fullfulment block thakbe

    if(subOrder.masterOrder.status !== "PAID"){
        throw new ApiError(400,"","Master order is not paid yet. Cannot update sub-order status.");
    }
